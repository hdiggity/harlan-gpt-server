...
# v4.0.0-instruct checkpoint + drinks-24576 tokenizer + retrieval-v4 corpus in the
# harlan-gpt model repo (READ-ONLY; that repo is never modified from here).
# ask_server_v4.py forces CPU inference (config.device="cpu"), so it never
# contends with a live MPS train.py run. Serving v4.0.0-instruct since 2026-07-19.
...
exec /Users/harlan/miniconda3/envs/harlan-gpt/bin/python scripts/ask_server_v4.py \
  --ckpt_dir /Users/harlan/services/harlan-gpt/whisky-gpt/gpt/runs/v4.0.0/finetuning/checkpoints \
